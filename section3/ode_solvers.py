...
DTS = [0.1, 0.01]                  # the two step sizes drawn in parts (a), (c), (d)
...
def energy(ys, omega=OMEGA):
    """H = p^2/2 + omega^2 q^2/2 for states of shape (..., 2)."""
    return 0.5 * ys[..., 1]**2 + 0.5 * omega**2 * ys[..., 0]**2
...
def main():
    parser = argparse.ArgumentParser(
        description="Section 3 ODE solvers -- run one part, or all of them."
    )
    parser.add_argument(
        "--part", choices=["a", "b", "c", "d", "all"], required=True,
        help="Which part of the problem to run.",
    )
    args = parser.parse_args()

    dispatch = {"a": part_a, "b": part_b, "c": part_c, "d": part_d}
